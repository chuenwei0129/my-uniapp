...
<template>
  ...
  >
    ...
    <!-- 错误处理，可在此替换 -->
    <view class="uqrcode-error" v-if="inError"
      ><text class="uqrcode-error-message">Error, see console.</text></view
    >
  </view>
</template>
...
export default {
  ...
  mounted() {
    this.templateOptions.size =
      this.sizeUnit == "rpx" ? uni.upx2px(this.size) : this.size;
    this.templateOptions.canvasWidth = this.templateOptions.size;
    this.templateOptions.canvasHeight = this.templateOptions.size;
    if (this.type == "2d") {
      // #ifndef MP-WEIXIN
      this.templateOptions.canvasTransform = `scale(${
        this.templateOptions.size / this.templateOptions.canvasWidth
      }, ${this.templateOptions.size / this.templateOptions.canvasHeight})`;
      // #endif
    } else {
      this.templateOptions.canvasTransform = `scale(${
        this.templateOptions.size / this.templateOptions.canvasWidth
    ...
    async draw(callback = {}, isDrawDelegate = false) {
      ...
      if (typeof this.uqrcodeOptions.errorCorrectLevel === "string") {
        this.uqrcodeOptions.errorCorrectLevel =
          UQRCode.errorCorrectLevel[this.uqrcodeOptions.errorCorrectLevel];
      }
      ...
      if (this.type === "2d") {
        ...
        canvasContext = this.canvasContext = uni.createCanvasContext(
          this.canvasId,
          this
        );
          ...
          return new Promise((resolve, reject) => {
            if (src.startsWith("http")) {
              ...
              if (src.startsWith(".")) {
                ...
                throw new Error(
                  "[uQRCode]: local image path only supports absolute path!"
                );
              } else {
                resolve(src);
              }
