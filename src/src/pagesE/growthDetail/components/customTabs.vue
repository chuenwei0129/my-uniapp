        ...
        >
              ...
              <view
                ...
                <u-badge
                  :show="
                    !!(
                      item.badge &&
                      (item.badge.show || item.badge.isDot || item.badge.value)
                    )
                  "
          ...
          <view
            ...
            :style="[
              {
                width: $u.addUnit(lineWidth),
                transform: `translateX(${lineOffsetLeft}px) translateY(-10px)`,
...
export default {
      ...
      return (index) => {
        ...
        const customeStyle =
          index === this.innerCurrent
            ? uni.$u.addStyle(this.activeStyle)
            : uni.$u.addStyle(this.inactiveStyle);
  ...
  methods: {
    setLineLeft() {
      const tabItem = this.list[this.innerCurrent || 0];
      if (!tabItem) {
        return;
      }
      // 获取滑块该移动的位置
      ...
      this.lineOffsetLeft =
        lineOffsetLeft + (tabItem.rect.width - lineWidth) / 2;
      // this.lineOffsetLeft = this.lineOffsetLeft - !!(item.badge && (item.badge.show || item.badge.isDot || item.badge.value)) ? 60 : 0;
      // #ifdef APP-NVUE
      // 第一次移动滑块，无需过渡时间
      this.animation(
        this.lineOffsetLeft,
        this.firstTime ? 0 : parseInt(this.duration)
      );
      // #endif
    ...
    setScrollLeft() {
      ...
      scrollLeft = Math.min(
        scrollLeft,
        this.scrollViewWidth - this.tabsRect.width
      );
    ...
    getTabsRect() {
      return new Promise((resolve) => {
        this.queryRect("u-tabs__wrapper__scroll-view").then((size) =>
          resolve(size)
        );
      });
    },
