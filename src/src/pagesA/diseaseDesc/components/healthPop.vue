  ...
  <view class="health-pop" @touchmove.stop.prevent="stopPrevent">
    ...
    >
      <view class="content">
        <view class="top-nav">
          <view class="title">{{
            healthData[healthType - 1] && healthData[healthType - 1].healthTitle
          }}</view>
        ...
        <view class="content-main">
          <view
            :class="{
              'no-border':
                index === healthChildren && healthChildren.length - 1,
            }"
...
export default {
  ...
  methods: {
    ...
    async handleSpecialEcho() {
      ...
      res?.data?.forEach(async (specialItem, index) => {
        // 特殊时期
    ...
    async handleEchoData() {
      this.oldHealthData =
        this.healthData && JSON.parse(JSON.stringify(this.healthData));
    ...
    openSecondPop(item) {
      ...
      console.log("🚀 ~ openSecondPop ~ item:", item);
    ...
    validateFile() {
      ...
      for (let i = 0; i < data.length; i++) {
        if (this.healthType == 3 && !this.selectedParams.weight) {
          toast = data[1].label;
          break;
        } else if (
          data[i].type === "weight" &&
        ...
        if (
          this.healthType !== 3 &&
          (!data[i].name || data[i].name == "" || data[i].name == null)
        ) {
          toast = data[i].label;
          break;
        }
