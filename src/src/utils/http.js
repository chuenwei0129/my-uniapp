...
export const BASR_URL =
  process.env.NODE_ENV === "development"
    ...
    : "https://cn-prod01-gw.chongpangpang.com/";
...
class Http {
  request({
    ...
    const info =
      uni.getStorageSync("storage_info") || store.state.userInfo || {};
...
function redirecLogOut() {
  ...
  const { route, options } = currentPage; // 当前页面的路由信息
  console.log(currentPage, options);
  if (options.source == "30") {
    uni.showModal({
      title: "提示",
      content: "获取用户凭证失效，请退出重新进入。",
      success: (res) => {
        window.paH5Utils.back();
      },
      fail: (err) => {
        window.paH5Utils.back();
      },
