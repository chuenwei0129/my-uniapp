<template>
  <view>
    <view class="location-container" v-if="isShowLoaction">
      <view class="cur-location" @click="handle2AddressPage">
        <!-- <i class="iconfont icon-address" style="font-size: 35rpx"></i> -->
        <image
          src="https://frontend-cdn.chongpangpang.com/image/medical-mp/index2/loaction-icon.png"
          style="width: 40rpx; height: 40rpx"
        ></image>
        <view class="location-name" v-if="curAddress"> {{ curAddress }}</view>
        <view v-else class="location-name">
          {{ noLocalTips }}
        </view>
        <!-- <u-icon name="arrow-down-fill" color="#1F1F1F" size="10"></u-icon> -->
        <image
    ...
    <view v-if="!myOpenSetting" class="location-fail">
      <view class="location-desc">
        <!-- <image
        src="https://frontend-cdn.chongpangpang.com/image/medical-mp/mall/in_store_service_location.png"
      >
      </image> -->
        <view>打开位置权限，查看附近的内容</view>
      </view>
      <view class="location-button" @tap="$u.debounce(handleOpenSetting, 500)"
        >开启
      </view>
    </view>
  </view>
...
export default {
  ...
  methods: {
    ...
    async searchLocation(data, refresh = true, saveflag) {
      ...
      let item = Object.assign(res.data, _obj);
      // let item = {
      //   lat: data.lat,
      //   lon: data.lon,
      //   address: address,
      ...
      uni.navigateTo({
        ...
        events: {
          resultEvent: async (res) => {
            ...
            let { addressName, address, lat, lon } = res.data;
            this.curAddress = address;
            this.location = {
              lat: lat,
              lon: lon,
    ...
    getMyAddress() {
      ...
      if (this.myOpenSetting) {
        ...
        //   saveAddress = true;
        // }
        let { lat, lon, address, city } = lastSearchAddressObj;
        if (lat && lon) {
