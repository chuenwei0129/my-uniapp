  ...
  <view class="sheets-item">
    ...
    <view class="sheets-info">
      <view class="info">
        ...
        <view
          :class="[
            sheet.type == 'QUICK'
              ? 'quick'
              : sheet.type == 'NUTRITION'
              ? 'nutrition'
              : sheet.type == 'ONLINE_PRESCRIPTION'
              ? 'online'
              : sheet.type == 'REPORT_INTERPRETATION'
              ? 'reports'
              : 'expert',
            'type',
          ]"
        ...
        <view class="sheets-status">{{
          sheet.prescriptionStatus === "EXPIRED"
            ? "已过期"
            : sheet.prescriptionStatus === "ISSUED"
            ? "已开具"
            : "已执行"
        }}</view>
      </view>
      ...
      <view class="btn-con">
        <view
          class="btn"
          v-if="sheet.prescriptionStatus === 'ISSUED'"
          @click.stop="
            () => {
              goBuyMedicines(sheet);
            }
          "
          >去购药</view
        >
        <view
          class="btn"
          v-if="sheet.prescriptionStatus === 'EXECUTED'"
          @click.stop="
            () => {
              goBuyMedicines(sheet);
            }
        ...
        <view
          class="btn"
          v-if="sheet.prescriptionStatus !== 'EXPIRED'"
          @click.stop="
            () => {
  ...
  methods: {
    ...
    goBuyMedicines() {
      if (this.sheet.orderNo) {
        action_report({
          action_name: "myrx_list_order_clk",
          module_name: "medical",
        });
        uni.navigateTo({
    ...
    goSubsequent() {
      action_report({
        action_name: "myrx_list_rx_clk",
        module_name: "medical",
      });
      //#ifdef MP-WEIXIN
      this.setHistoryPrescription(this.sheet);
      // uni.navigateTo({
      //   url: `/pagesB/subsequent/index?petId=${this.sheet.petId}`,
      // });
      //列表页面跳转到引导页面
      uni.navigateTo({
        url: `/pagesA/subsequent/index?petId=${this.sheet.petId}`,
      });
      //#endif
      
      //#ifdef H5
      uni.navigateTo({
        url: `/pagesA/subsequent/index?petId=${this.sheet.petId}&diagnosticId=${this.sheet.id}`,
      });
      //#endif
    },
    /**
    ...
    diagnosisDetails(item) {
      action_report({
        action_name: "myrx_list_view_clk",
        module_name: "medical",
      });
      uni.navigateTo({
