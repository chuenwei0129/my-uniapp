<template>
    <view class="customer-banner-wrap" :style="showStyle" v-if="bannerList.length">
        <swiper class="swiper" circular :autoplay="true" :interval="5000" :duration="500" @change="handleAdChanged" style="height: 154rpx;border-radius: 23rpx;overflow: hidden;">
            <swiper-item v-for="(item, index) in bannerList" :key="index" style="height: 154rpx;border-radius: 23rpx;overflow: hidden">
                <view class="swiper-item" @click="bannerJump(item, index)">
                    <image style="width: 100%; height: 154rpx;" :src="item.material.url" mode="aspectFill"></image>
                </view>
            </swiper-item>
        </swiper>
        <!-- 面板指示点 -->
