<template>
  <view class="filter-wrapper">
    <view class="tab-list">
      <view
        :class="['item', isSelected(1) ? 'active' : '']"
        @click="handleSelect(1)"
      >
        综合
      </view>
      <view
        :class="['item', isSelected(2) ? 'active' : '']"
        @click="handleSelect(2)"
      >
        销量
      </view>
      <view
        :class="['item', isSelected(3) || isSelected(4) ? 'active' : '']"
        @click="handleSelect(sortType === 3 ? 4 : 3)"
      >
        <view>价格</view>
        <view :class="priceIcoClass" />
      </view>
    </view>
    <view
      ...
      :style="{
        bottom: navBarHeight
          ? `calc((70rpx + ${navBarHeight}px) / 2 + 3rpx)`
          : 0,
      }"
...
export default {
  ...
  mounted () {
    ...
    // #endif
    uni.$off('addCardNumActive')
    uni.$on('addCardNumActive', () => {
  ...
  methods: {
    ...
    handleCartTapped () {
      ...
      uni.navigateTo({
        ...
        url: '/pagesH/inStoreService/index',
      })
