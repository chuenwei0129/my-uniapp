<template>
  <view class="y-tabs" :class="[uniqueTabsClass, tabsClass]">
    ...
    <view class="y-tabs__sticky" :style="[stickyStyle]">
      <view
        class="y-tabs__wrap"
        :style="[innerWrapStyle, wrapStyle]"
        :class="[wrapClass]"
      >
        <!-- 标签栏左侧插槽 -->
          ...
          <view class="y-tabs__nav" :class="[navClass]" :style="[navStyle]">
            ...
            >
              ...
              <view class="y-tab__info" v-if="tab.dot || tab.badge">
                ...
                <!-- 徽标 -->
                <text class="y-tab__info--badge" v-if="tab.badge">{{
                  formatBadge(tab)
                }}</text>
              </view>
            </view>
...
export default {
  ...
  computed: {
    ...
    scrollX() {
      return (
        !this.isVertical && this.tabs.length > Number(this.scrollThreshold)
      );
    },
    // js模式时，是否处于吸顶模式
    jsFixed() {
      ...
      if (this.sticky) {
        if (this.cssSticky) {
          ...
          ["width", "height"].forEach(
            (key) => (style[key] = addUnit(this.stickyRect[key]))
          );
        }
      } else {
        // 无需吸顶时，设置relative定位
    ...
    innerWrapStyle() {
      ...
      if (this.isVertical && this.isFixed) {
        ...
        style.height = addUnit(
          Math.min(this.fixedRelativeHeight, this.wrapRect.height)
        );
      }
      return style;
    },
    ...
    barInnerStyle() {
      const style = {};
      // 水平和垂直的处理
      style.transition = this.barAnimated
        ? `all ${this.duration}s linear`
        : "none";
      const width = !isNull(this.barCalcedWidth)
        ? addUnit(this.barCalcedWidth)
        : null;
      const height = !isNull(this.barCalcedHeight)
        ? addUnit(this.barCalcedHeight)
        : null;
    ...
    currentIndex(newIdx, oldIdx) {
      ...
      if (this.scrollspy)
        this.childrens.forEach(
          (child, index) => (child.isDisjoint = index < newIdx)
        );
  ...
  methods: {
    ...
    putTab(props) {
      ...
      this.tabs.push({
        ...
        titleSlotName: this.getTitleSlotName(
          defaultSlotName,
          props?.titleSlotName
        ), //标题的实际插槽名
    ...
    updateTab(props, index) {
      const tab = this.tabs[index];
      Object.entries(props || {}).forEach((ele) =>
        this.$set(tab, ele[0], ele[1])
      );
      this.$set(
        tab,
        "titleSlotName",
    ...
    titleStyle(tab) {
      ...
      // 标题选中时和默认时的样式
      const titleStyle = activated
        ? this.titleActiveStyle
        : this.titleInactiveStyle;
      const style = {
    ...
    formatBadge(tab) {
      if (
        !isNull(tab?.badge) &&
        !isNull(tab?.badgeMaxCount) &&
        tab?.badge > tab?.badgeMaxCount
      ) {
    ...
    // 销毁观察器
    destroyObserver() {
      ["dependObserver", "tranObserver"].forEach((name) =>
    ...
    async observeSticky() {
      ...
      // 绑定观察的元素
      dependObserver.observe(
        `.${this.uniqueTabsClass} .y-tabs__depend`,
        (res) => {
          // 判断是否达到吸顶条件范围
          this.isFixed =
            res.intersectionRatio <= 0 &&
            res.boundingClientRect.top <= offsetTop;
          // 标签栏垂直吸顶时，获取参照节点高度
          if (this.isVertical && this.isFixed)
    ...
    async observerTransparent() {
      ...
      tranObserver.observe(".y-tabs__depend--transparent", (res) => {
        ...
        let opacity =
          res.boundingClientRect.top > res.relativeRect.top
            ? res.intersectionRatio + A
            : 1;
    ...
    async observePane() {
      ...
      const top = this.pageScroll
        ? Number(this.offsetTop) +
          (this.scrollNav ? this.wrapRect?.height || 0 : 0)
    ...
    setCurrentIndexByName(name) {
      const matched = this.childrens.find(
        (child) => child.computedName === name
      );
    ...
    handleTransparentColor(ms = 0) {
      ...
      this.intervalFn(
        async (clear) => {
          ...
          if (ms > 0) {
            let opacity =
              scrollTop < this.transparentOffset ? scrollTop * 0.01 : 1;
            if (opacity < 1) ms -= 50;
            else ms = 0;
    ...
    changeStatus(newIdx, oldIdx) {
      ...
      // 子数组标记激活状态
      this.childrens.forEach(
        (child, index) => (child.active = newIdx === index)
      );
    },
    // 样式切换
    async changeStyle() {
    ...
    getTabScrollOffset() {
      return new Promise((resolve, reject) => {
        ...
        const nodeRef = this.getSelectorQuery().select(
          `.${this.uniqueTabsClass} .y-tabs__scroll`
        );
    ...
    async updateTabRect(isFull = false) {
      ...
      if (isFull) {
        ...
        const isInit =
          this.scrollDetail?.scrollLeft === 0 &&
          this.scrollDetail?.scrollTop === 0;
        // 获取tab的节点信息
        const rects = await this.getBatchRect(
          this.tabs.map((_, i) => `.y-tab_${i}`)
        );
        this.tabRects = this.tabs.map((tab, index) => {
          ...
          result.leftSpace =
            result.left - (index === 0 ? this.scrollViewRect?.left : prevRight);
          result.topSpace =
            result.top - (index === 0 ? this.scrollViewRect?.top : prevBottom);
          return result;
        });
      } else {
        ...
        idxs.forEach((i, idx) => {
          const matched = this.tabRects?.[i],
            rect = rects?.[idx];
          if (matched && rect)
            keys.forEach((key) => this.$set(matched, key, rect?.[key]));
        });
      }
      ...
      tabRects.forEach((item, i) => {
        ...
        if (!this.isVertical) {
          this.$set(
            ...
            item.index === 0
              ? item.leftSpace
              : prev.offsetLeft + prev.width + item.leftSpace
          );
        } else {
          this.$set(
            item,
            ...
            item.index === 0
              ? item.topSpace
              : prev.offsetTop + prev.height + item.topSpace
          );
        }
        const res = this.getTabOffset(item);
        this.$set(item, "barOffset", res.barOffset);
    ...
    getTabOffset({ width, height, offsetLeft, offsetTop }) {
      if (!this.isVertical) {
        ...
        const barOffset =
          this.isLine && !this.autoLineBarWidth
            ? cLeft - this.barRect.width / 2
            : offsetLeft;
        ...
        const barOffset =
          this.isLine && !this.autoLineBarHeight
            ? cTop - this.barRect.height / 2
            : offsetTop;
    ...
    async touchEndForPane(deltaX, currIndex, targetIndex, isChange) {
      ...
      const isAnimatedMode =
        !this.isVertical && (this.isLineMode || this.isWormMode);
          ...
          if (rate < extendRange) {
            ...
            if (!isRight) {
              barOffset =
                currOffset +
                extendWidth +
                (shrinkWidth / shrinkRange) * (dxRate - extendRange);
            } else {
    ...
    getRect(selector) {
      return new Promise((resolve, reject) => {
        ...
        query
          .select(`.${this.uniqueTabsClass} ${selector}`)
          .boundingClientRect();
